...
parser.add_option('-i', '--triage_csv', help='Specifies local file containing [link, key, summary, assignee, reviewer, status, prio, repro, scope, component] triage file to update against live JIRA data')
parser.add_option('-o', '--triage_out', help='Output file name for updated triage data. If not provided, prints to stdout.')
parser.add_option('-c', '--to_clean', help='Input CSV file w/2nd column representing JIRA tickets to close all resolved tickets from.')
...
parser.add_option('-a', '--interactive', help='Default mode: run interactive console menu', action='store_true', dest='interactive')
